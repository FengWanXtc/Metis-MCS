...
def main():
    ...
    # 设置对应的MCS
    data = read_json_file('RTMP_Pull_Student.json')
    data['Name'] = MCSNAME
    data["CodecStreamSpecs"][0]["URI"] = "rtmp://{}/test/ch3".format(IP)
    ...
    if response.json()['Code'] == 201 or response.json()['Code'] == 202:
        print('Compose Stream RTMP Student Pull Pass!')
    else:
        print('Compose Stream RTMP Student Pull Fail!')
        print(response.json())
