...
def main():
    ...
    # 设置对应的MCS
    data = read_json_file('RTMP_Push_Student.json')
    data['Name'] = MCSNAME
    data["ServerSpecs"][0]["URI"] = "rtmp://{}/test/ch3".format(TARGET)
    ...
    if response.json()['Code'] == 201 or response.json()['Code'] == 202:
        print('Compose Stream RTMP Student Push Pass!')
    else:
        print('Compose Stream RTMP Student Push Fail!')
        print(response.json())
