...
def rtsp_push(IP):
    ...
    for VideoIn in VideoIns:
        if VideoIn['DeviceName'].find(Global_GeneralCamType) != -1:
            ...
            break
    # 找到第二种类型摄像头获取DeviceId
    for VideoIn in VideoIns:
        if VideoIn['DeviceName'].find(Global_GeneralCamType_other) != -1:
            rtsp_push_data['VideoSpecs'][1]['DeviceId'] = VideoIn['DeviceId']
            break

    if IP == '':
        # IP 为空说明是在当前小站
        rtsp_push_data['ServerSpecs'][0]['URI'] = 'rtsp://127.0.0.1:8554/metis/ch1'
        rtsp_push_data['ServerSpecs'][1]['URI'] = 'rtsp://127.0.0.1:8554/metis/ch2'
    else:
        # IP 不为空需要指定推流小站
        rtsp_push_data['ServerSpecs'][0]['URI'] = 'rtsp://{}:8554/metis/ch1'.format(IP)
        rtsp_push_data['ServerSpecs'][1]['URI'] = 'rtsp://{}:8554/metis/ch2'.format(IP)
