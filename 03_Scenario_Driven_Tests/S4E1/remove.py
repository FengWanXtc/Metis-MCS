...
def push_record(argc_ip, mcsname, specName):
    ...
    try:
        ...
        print('Remove MCS ({}) SpecName {} Failed'.format(mcsname, specName))
        print(response.status_code)
        return False

    if response.json()['Code'] == 200:
        print('Remove MCS ({}) SpecName ({}) Success'.format(mcsname, specName))
        return True
    else:
        print('Remove MCS ({}) SpecName ({}) Fail'.format(mcsname, specName))
        print(response.json())
        return False
...
def main():
    ...
    parser.add_argument('--mcsname', '-mn', type=str, default='MCSV3', help='The name of MCS,default is MCSV3')
