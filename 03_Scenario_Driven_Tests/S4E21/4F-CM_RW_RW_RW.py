import argparse
import sys


sys.path.append('../../Libraries')
from mediatasklib import *
from functionlib import *
from mediadevicelib import get_md_audio_in


def S4E3_Basic():
    # 设置对应的MCS
    parser = argparse.ArgumentParser(description='usage: python3 delete_all_tasks --ip=10.12.224.135')
    parser.add_argument('-ip', type=str, default='', help='The IP address of the METIS')
    args = parser.parse_args()
    IP = args.ip

    S4E3_data = read_json_file('4F-CM_RW_RW_RW.json')
    # CAM1、CAM2、CAM_IN 的 DeviceId基本已定,需修改的是IPC
    Teacher_DeviceId = get_DeviceId_by_aiStrategy(ip=IP, camType='IPC', aiStrategy='teacher')
    S4E3_data['VideoSpecs'][0]['DeviceId'] = Teacher_DeviceId

    Student_DeviceId = get_DeviceId_by_aiStrategy(ip=IP, camType='IPC', aiStrategy='student')
    S4E3_data['VideoSpecs'][1]['DeviceId'] = Student_DeviceId
    Blackboard_DeviceId = get_DeviceId_by_aiStrategy(ip=IP, camType='IPC', aiStrategy='blackboard')
    S4E3_data['VideoSpecs'][3]['DeviceId'] = Blackboard_DeviceId
    # print(json.dumps(S4E3_data, indent=2))
    AudioIns = get_md_audio_in(ip=IP).json()["AudioIns"]
    # 配置 MIC 和 PC In 的Audio DeviceId
    for AudioIn in AudioIns:
        if AudioIn["DeviceName"].find("BuiltIn USB Audio") != -1:
            S4E3_data["AudioSpecs"][0]["DeviceId"] = AudioIn["DeviceId"]
        if AudioIn["DeviceName"].find("BuiltIn HDMI Audio") != -1:
            S4E3_data["AudioSpecs"][1]["DeviceId"] = AudioIn["DeviceId"]
    # print(json.dumps(S4E3_data, indent=2))
    try:
        response = post_mt_create_update(ip=IP, data=S4E3_data)
        # print(response.url)

    except:
        print('Post Create Task Failed.')
        print(response)

    if response.json()['Code'] == 201:
        print('S4E21 Basic Test Pass!')
    else:
        print('S4E21 Basic Test Fail!')
        print(response.json())


if __name__ == '__main__':
    S4E3_Basic()
