...
def push_record(argc_ip, mcsname, specName):
    ...
    try:
        response = post_mt_resume(ip=argc_ip, data=data)
    except:
        print('Resume :: MCS ({}) SpecName {} Failed'.format(mcsname, specName))
        print(response.status_code)
        return False

    if response.json()['Code'] == 200:
        print('Resume :: MCS ({}) SpecName ({}) Success'.format(mcsname, specName))
        return True
    else:
        print('Resume :: MCS ({}) SpecName ({}) Fail'.format(mcsname, specName))
        print(response.json())
        return False
