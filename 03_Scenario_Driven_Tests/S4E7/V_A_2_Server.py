...
def main():
    ...
    # 设置对应的MCS
    data = read_json_file('V_A_2_Server.json')
    data['Name'] = MCSNAME
    data["ServerSpecs"][0]["URI"] = "wp://{}:6671".format(TARGET)
    data["ServerSpecs"][1]["URI"] = "wp://{}:6673".format(TARGET)
    data["ServerSpecs"][2]["URI"] = "wp://{}:6674".format(TARGET)
    data["ServerSpecs"][3]["URI"] = "wp://{}:6675".format(TARGET)
    data["ServerSpecs"][4]["URI"] = "wp://{}:6676".format(TARGET)
    ...
    if response.json()['Code'] == 201:
        print('Far End 1 Video With Audio Push Pass!')
    else:
        print('Far End 1 Video With Audio Push Fail!')
        print(response.json())
