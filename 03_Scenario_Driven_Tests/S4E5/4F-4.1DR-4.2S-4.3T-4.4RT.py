...
def S4E3_Basic():
    ...
    Blackboard_DeviceId = get_DeviceId_by_aiStrategy(ip=IP, camType=Global_BlackBoardCamType, aiStrategy='blackboard')
    ...
    if response.json()['Code'] == 201 or response.json()['Code'] == 202:
        print('S4E3 Basic Test Pass!')
    else:
        print('S4E3 Basic Test Fail!')
        print(response.json())
