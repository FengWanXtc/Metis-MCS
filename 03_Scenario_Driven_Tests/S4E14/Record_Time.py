...
def basic():
    ...
    parser.add_argument('--unit', '-u', type=int, default=10, help='The Split Unit (Seconds)')
    ...
    data = read_json_file('Record_Time.json')
    data['RecordSpecs'][0]['SplitUnit'] = UNIT
    data['RecordSpecs'][0]['Path'] = "WP4_TimeSplit_{}_yyyy-mm-dd-hh-mm-ss.mp4".format(str(FILENAME))
    ...
    if response.json()['Code'] == 201:
        print('Record Split By Time Test Pass!')
    else:
        print('Record Split By Time Test Fail!')
        print(response.json())
