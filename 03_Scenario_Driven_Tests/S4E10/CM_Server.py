...
def main():
    ...
    # 设置对应的MCS
    data = read_json_file('CM_Server.json')
    data['Name'] = MCSNAME
    data["ServerSpecs"][0]["URI"] = "wp://{}:6673".format(TARGET)
    data["ServerSpecs"][1]["URI"] = "wp://{}:6674".format(TARGET)
    ...
    try:
        ...
        print('Compose Stream Push Failed.')
        print(response.status_code)
    # print(response.status_code)

    if response.json()['Code'] == 201 or response.json()['Code'] == 202:
        print('Compose Stream Push Pass!')
    else:
        print('Compose Stream Push Fail!')
        print(response.json())
