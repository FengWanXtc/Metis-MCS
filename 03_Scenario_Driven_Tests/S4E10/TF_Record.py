...
def main():
    ...
    # 设置对应的MCS
    data = read_json_file('TF_Record.json')
    data['Name'] = MCSNAME
    # 录制的文件名
    data["RecordSpecs"][0]["Path"] = "S4E10_{}_Transform_yyyy-mm-dd-hh-mm-ss.mp4".format(RecordName)
    ...
    try:
        ...
        print('Transform {} Record Failed.'.format(RecordName))
        print(response.status_code)
    # print(response.status_code)

    if response.json()['Code'] == 201 or response.json()['Code'] == 202:
        print('Transform {} Record Pass!'.format(RecordName))
    else:
        print('Transform {} Record Fail!'.format(RecordName))
        print(response.json())
