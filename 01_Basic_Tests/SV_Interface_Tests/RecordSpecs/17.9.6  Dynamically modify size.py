...
def S4E7_Basic():
    ...
    try:
        ...
        print(response)
    time.sleep(10)
    print("动态修改分段录制大小为3145728（3M左右）,需要观察录制生成的问题是否还是以3M生成，大约需要等待1分钟以上")
    data["RecordSpecs"][0]["SplitUnit"] = 3145728
    data["RecordSpecs"][1]["SplitUnit"] = 3145728
    data["RecordSpecs"][2]["SplitUnit"] = 3145728
    data["RecordSpecs"][3]["SplitUnit"] = 3145728
    try:
        response = post_mt_create_update(ip=IP, data=data)
        # print(response.json())
    except:
