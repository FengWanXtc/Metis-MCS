...
r = requests.post(data2["url"], headers=data2["header"], json=data2['body'])
time.sleep(5)
true = True
false = False
...
data = {
    "times": TIME,  # 并发量
    # "method": "POST",
    "url": "http://{}:6689/mediatask/metis_live_mcs/Director".format(IP),
    "header": {'Content-type': 'application/json'},
    "body": {}
}
...
def get_requests(d):
    ...
    global ERROR_NUM
    try:
        r = requests.get(d["url"], headers=d["header"], json=d['body'])
        # print(r.status_code)
        if r.json()['Code'] == 200:
            lock.acquire()  # 上锁
            RIGHT_NUM += 1
