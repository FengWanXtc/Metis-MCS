...
def create_preview(IP, device, mcsname, name, position):
    cre_data = read_json_file('create_preview_with_record.json')
    ...
    if response.json()['Code'] == 201:
        print('Create one frame preview {} With Record Success'.format(name))
        return True
    else:
        print('Create one frame preview {} With Record Fail'.format(name))
        print(response.json())
