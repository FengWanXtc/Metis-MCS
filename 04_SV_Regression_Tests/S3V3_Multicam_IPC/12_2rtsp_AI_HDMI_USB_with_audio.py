...
if __name__ == '__main__':
    act(scriptlist)
    # 删除2路IPC摄像头
    input(Anno + "\nPlease Press Enter Key to *Delete IPC1 IPC2 IPC3* Task.")
    os.system('python delete_task_by_name.py -n=IPC1 -ip={}'.format(IP))
    os.system('python delete_task_by_name.py -n=IPC2 -ip={}'.format(IP))
    os.system('python delete_task_by_name.py -n=IPC3 -ip={}'.format(IP))
    # 创建2路带AI 导播教师(HDMI+IPC)
    input(Anno + "\nPlease Press Enter Key to *Create AI 3 Frame* Task.")
    os.system('python IPC_AI_D_T_S_3.py -ip={}'.format(IP))

    # 删除导播任务
    input(Anno + "\nPlease Press Enter Key to Delete Director Tasks.")
    os.system('python delete_task_by_name.py -n=IPC_AI_D_3'.format(IP))

    # 开启加audio推流录制任务
    input(Anno + "\nPlease Press Enter Key to  *Create AI 3 Frame with Audio* Tasks.")
    os.system('python IPC_AI_D_T_S_3_with_audio.py -ip={}'.format(IP))
