...
def create_preview(IP, device, mcsname, name, position):
    cre_data = read_json_file('create_preview_push_pull.json')
    cre_data["VideoSpecs"][0]["Name"]  = name
    ...
    if response.json()['Code'] == 201:
        print('Create one frame push pull preview {} With Audio Success'.format(name))
        return True
    else:
        print('Create one frame push pull preview  {} With Audio Fail'.format(name))
        print(response.json())
        # print(json.dumps(cre_data, indent=2))
        return False
