...
def rtc_dynamic(task_name, host_ip):
    ...
    Dynamic_Rtc_MCS['RtcStreamSpec']['AudioCodecName'] = 'Compositor_h264_Codec'

    post_announce(host_ip=host_ip, mcs_data=Dynamic_Rtc_MCS,
                  announce="Compose画中画MCS下发成功，检查效果后按回车进行下一个步骤 :: 动态修改ComposeSpec,画中画改为教师+学生")



    response = post_mt_create_update(ip=host_ip, data=Dynamic_Rtc_MCS)
    if response.json()['Code'] == 201:
        input("Compose画中画MCS下发成功，检查效果后按回车进行下一个步骤 :: 动态修改ComposeSpec,画中画改为教师+学生")
    else:
        print(response.json())
        raise UserWarning("创建任务失败，请检查环境")
    ...
    response = post_mt_create_update(ip=host_ip, data=Dynamic_Rtc_MCS)
    if response.json()['Code'] == 201:
        input("修改画中画MCS下发成功，检查效果后按回车进行下一个步骤 :: 动态修改ComposeSpec,画中画改为导播+PC")
    else:
        print(response.json())
        raise UserWarning("创建任务失败，请检查环境")
    ...
    response = post_mt_create_update(ip=host_ip, data=Dynamic_Rtc_MCS)
    if response.json()['Code'] == 201:
        input("修改画中画MCS下发成功，检查效果后按回车进行下一个步骤 :: 动态修改ComposeSpec,画中画改为二分屏")
    else:
        print(response.json())
        raise UserWarning("创建任务失败，请检查环境")
    ...
    response = post_mt_create_update(ip=host_ip, data=Dynamic_Rtc_MCS)
    if response.json()['Code'] == 201:
        input("二分屏MCS下发成功，检查效果后按回车进行下一个步骤 :: 动态修改二分屏")
    else:
        print(response.json())
        raise UserWarning("创建任务失败，请检查环境")
